...
export default function CargaUsuarios() {
  ...
  const onChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await clienteAxios.post("/api/usuarios/cargar-formato", data);
  ...
  const descargarFormato = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setDescargando(true);
    try {
      const res = await clienteAxios.get("/api/usuarios/descargar-formato", {
        responseType: "blob",
      });
      if (res.data) {
  ...
  const enviar = async (e: React.MouseEvent<HTMLButtonElement>) => {
    try {
      ...
      if (res.data.estado) {
        const { usuarios, correos, registros } = res.data.datos;
        enqueueSnackbar(`Usuario creados: ${usuarios}`, { variant: "success" });
        if (envioCorreos)
            ...
            <Fragment>
              ...
              {descargando ? (
                ...
                >
                  <Box sx={{ width: "100%" }}>
                    <FormControlLabel
                      label="Enviar correos a usuarios"
                      control={
                        <Checkbox
                          name="envioCorreos"
