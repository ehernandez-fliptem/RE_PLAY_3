...
export default function Cubiculos() {
      ...
      getRows: async (params) => {
        ...
        try {
          ...
          const res = await clienteAxios.get(
            "/api/cubiculos?" + urlParams.toString()
          );
          if (res.data.estado) {
            setError("");
  ...
  const nuevoRegistro = () => {
    navigate("nuevo-cubiculo");
  };

  const editarRegistro = (ID: string) => {
    navigate(`editar-cubiculo/${ID}`);
  };

  const verRegistro = (ID: string) => {
    navigate(`detalle-cubiculo/${ID}`);
  };

  const cambiarEstado = async (ID: string, activo: boolean) => {
    if (!activo) {
      try {
        const res = await clienteAxios.patch(`/api/cubiculos/${ID}`, {
          activo,
        });
        if (res.data.estado) {
      ...
      confirm({
        title: "¿Seguro que deseas desactivar a este cubiculo?",
        description: "",
        allowClose: true,
    ...
    <div style={{ minHeight: 400, position: "relative" }}>
      <DataGrid
        ...
        slots={{
          toolbar: () => (
            <DataGridToolbar
              tableTitle="Gestión de Cubiculos"
              customActionButtons={
