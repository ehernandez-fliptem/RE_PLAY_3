...
export default function EditarPuesto() {
  ...
  useEffect(() => {
    const obtenerRegistro = async () => {
      try {
        const res = await clienteAxios.get(`/api/puestos/${ID}`);
        if (res.data.estado) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      const res = await clienteAxios.put(`/api/puestos/${ID}`, data);
      if (res.data.estado) {
        enqueueSnackbar("El puesto se modificó correctamente.", {
          variant: "success",
        });
