...
export default function Puestos() {
      ...
      getRows: async (params) => {
        ...
        try {
          ...
          const res = await clienteAxios.get(
            "/api/puestos?" + urlParams.toString()
          );
          if (res.data.estado) {
            setError("");
  ...
  const nuevoRegistro = () => {
    navigate("nuevo-puesto");
  };

  const editarRegistro = (ID: string) => {
    navigate(`editar-puesto/${ID}`);
  };

  const verRegistro = (ID: string) => {
    navigate(`detalle-puesto/${ID}`);
  };

  const cambiarEstado = async (ID: string, activo: boolean) => {
    if (!activo) {
      try {
        const res = await clienteAxios.patch(`/api/puestos/${ID}`, {
          activo,
        });
        if (res.data.estado) {
      ...
      confirm({
        title: "¿Seguro que deseas desactivar a este puesto?",
        description: "",
        allowClose: true,
