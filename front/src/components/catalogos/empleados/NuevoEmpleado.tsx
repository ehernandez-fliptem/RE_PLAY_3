    ...
    try {
      ...
      if (res.data.estado) {
        ...
        if (pendientes.length > 0) {
          enqueueSnackbar(
            `Empleado guardado, pero qued� pendiente sincronizar en: ${pendientes.join(", ")}.`,
            { variant: "warning" }
          );
        }
  ...
  const crearGrupoBiostarDesdeForm = async () => {
    ...
    try {
      ...
      await Swal.fire({
        icon: "success",
        title: "Grupo creado",
        text: "El grupo se cre� correctamente.",
        showConfirmButton: true,
        allowOutsideClick: false,
        ...swalTop,
        showClass: { popup: "swal2-show" },
      ...
      await Swal.fire({
        icon: "error",
        title: "No se pudo crear",
        text: error?.response?.data?.mensaje || error?.message || "Ocurri� un error al crear el grupo.",
        showConfirmButton: true,
        allowOutsideClick: false,
        ...swalTop,
        showClass: { popup: "swal2-show" },
