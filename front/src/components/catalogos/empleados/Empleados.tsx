...
export default function Empleados() {
  ...
  const abrirBiometria = async (
    ...
    try {
      if (step === "tarjeta" && !tarjetaHikiEnabled) {
        enqueueSnackbar("Tarjeta solo est谩 disponible con Hikvision activo.", {
          variant: "warning",
        });
        return;
      }
    ...
    try {
      ...
      if (res.data.estado) {
        ...
        if (haySiguienteProveedorHuella) {
          ...
          setBiometriaStep("huella");
          enqueueSnackbar(`Huella guardada en ${proveedorHuellaLabel}. Contin煤a con el siguiente sistema.`, {
            variant: "success",
          });
        } else {
          setBiometriaStep("ok");
  ...
  // [En proceso] Funci贸n de carga masiva deshabilitada temporalmente
  // const cargaMasiva = () => {
  ...
  const cambiarEstado = async (ID: string, activo: boolean, nombre: string) => {
    if (!activo) {
      confirm({
        title: "縎eguro que deseas restaurar a este empleado?",
        description: nombre,
        allowClose: true,
      ...
      confirm({
        title: "縎eguro que deseas desactivar a este empleado?",
        description: nombre,
        allowClose: true,
        confirmationText: "Continuar",
      })
  ...
  const eliminarPermanente = (ID: string, nombre: string) => {
    confirm({
      title: "縎eguro que deseas eliminar permanentemente este empleado?",
      description: nombre,
      allowClose: true,
      confirmationText: "Continuar",
    })
    ...
    try {
      ...
      if (res.data?.estado) {
        ...
        if (pendientes.length === 0) {
          enqueueSnackbar("Sincronizaci贸n completada.", { variant: "success" });
        } else {
          enqueueSnackbar(
            `Sincronizaci贸n pendiente en: ${pendientes.join(", ")}.`,
            { variant: "warning" }
          );
        }
        apiRef.current?.dataSource?.fetchRows?.();
      } else {
        enqueueSnackbar(res.data?.mensaje || "No se pudo reintentar la sincronizaci贸n.", {
          variant: "warning",
        });
      }
    } catch (error) {
  ...
  /*
  {
    ...
    display: "flex",
    minWidth: 80,
    // Ocultado porque no se necesita mostrar el ID en la gesti贸n de empleados por ahora.
  },
  {
    headerName: "Rol",
    field: "rol",
    ...
    display: "flex",
    minWidth: 120,
    // Ocultado porque el rol no es relevante para la gesti贸n directa de empleados en esta vista.
  },
  {
    headerName: "Tipo",
    field: "tipo",
    ...
    display: "flex",
    minWidth: 100,
    // Ocultado porque el tipo no se requiere en la gesti贸n de empleados actualmente.
  },
  {
    headerName: "Arco",
    field: "arco",
  ...
  },
  {
    ...
    display: "flex",
    minWidth:100,
    // Ocultado porque la gesti贸n de acceso no se requiere por ahora.
  },
  */
    ...
    <div style={{ minHeight: 400, position: "relative" }}>
      {pendingSyncCount > 0 && (
        <Alert severity="warning" sx={{ mb: 1 }}>
          Hay {pendingSyncCount} empleado(s) con sincronizaci贸n pendiente con integraciones.
        </Alert>
      )}
      <DataGrid
        apiRef={apiRef}
        ...
        slots={{
          toolbar: () => (
            <DataGridToolbar
              tableTitle="Gesti贸n de Empleados"
              customActionButtons={
                <Fragment>
                  ...
                  {/* [En proceso] Bot贸n de carga masiva oculto porque la funcionalidad a煤n no est谩 disponible
                        ...
                        <Tooltip
                          title={
                            puedeAgregarTarjeta
                              ...
                              : "No puedes agregar m谩s de 10 tarjetas. Borra una para continuar."
                          }
                        >
                          <span>
                            <Button
                      ...
                      >
                        {tarjetasWeb.length === 0 && (
                          <Box sx={{ p: 2, color: "text.secondary", fontSize: "0.92rem" }}>
                            A煤n no hay tarjetas registradas en este m贸dulo.
                          </Box>
                        )}
                          ...
                          >
                            <Box sx={{ pr: 2 }}>
                              ...
                              <Box sx={{ fontSize: "0.86rem", color: "text.secondary" }}>
                                {tarjeta.descripcion || "Sin descripci贸n"}
                              </Box>
