...
export default function Pisos() {
  ...
  const editarRegistro = (ID: string) => {
    navigate(`editar-piso/${ID}`);
  };

  const verRegistro = (ID: string) => {
    navigate(`detalle-piso/${ID}`);
  };

  const cambiarEstado = async (ID: string, activo: boolean) => {
    if (!activo) {
      try {
        const res = await clienteAxios.patch(`/api/pisos/${ID}`, {
          activo,
        });
        if (res.data.estado) {
      ...
      confirm({
        title: "¿Seguro que deseas desactivar a este piso?",
        description: "",
        allowClose: true,
