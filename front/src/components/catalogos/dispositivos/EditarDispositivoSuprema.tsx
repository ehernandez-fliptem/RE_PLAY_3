...
export default function EditarDispositivoSuprema() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const run = async () => {
      try {
        const res = await clienteAxios.get(`/api/dispositivos-suprema/form-editar/${id}`);
        if (res.data.estado) {
          formContext.reset({
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      ...
      const testRes = await clienteAxios.post(`/api/dispositivos-suprema/probar-conexion/${id}`, data);
      ...
      const res = await clienteAxios.put(`/api/dispositivos-suprema/${id}`, data);
      if (res.data.estado) {
        await Swal.fire({
          icon: "success",
          title: "Guardado",
