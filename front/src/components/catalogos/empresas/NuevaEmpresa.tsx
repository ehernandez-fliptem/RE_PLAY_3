...
export default function NuevaEmpresa() {
  ...
  useEffect(() => {
    const obtenerRegistro = async () => {
      try {
        const res = await clienteAxios.get("/api/empresas/form-nuevo/");
        if (res.data.estado) {
          const { pisos, accesos, puestos, departamentos, cubiculos } = res.data.datos;
          setPisos(pisos);
          setAccesos(accesos);
          setPuestos(puestos);
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      const res = await clienteAxios.post("api/empresas", data);
      if (res.data.estado) {
