...
export default function EditarDepartamento() {
  ...
  useEffect(() => {
    const obtenerRegistro = async () => {
      try {
        const res = await clienteAxios.get(`/api/departamentos/${ID}`);
        if (res.data.estado) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      const res = await clienteAxios.put(`/api/departamentos/${ID}`, data);
      if (res.data.estado) {
        enqueueSnackbar("El departamento se modificó correctamente.", {
          variant: "success",
        });
