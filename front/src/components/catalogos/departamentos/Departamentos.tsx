...
export default function Departamentos() {
      ...
      getRows: async (params) => {
        ...
        try {
          ...
          const res = await clienteAxios.get(
            "/api/departamentos?" + urlParams.toString()
          );
          if (res.data.estado) {
            setError("");
  ...
  const nuevoRegistro = () => {
    navigate("nuevo-departamento");
  };

  const editarRegistro = (ID: string) => {
    navigate(`editar-departamento/${ID}`);
  };

  const verRegistro = (ID: string) => {
    navigate(`detalle-departamento/${ID}`);
  };

  const cambiarEstado = async (ID: string, activo: boolean) => {
    if (!activo) {
      try {
        const res = await clienteAxios.patch(`/api/departamentos/${ID}`, {
          activo,
        });
        if (res.data.estado) {
      ...
      confirm({
        title: "¿Seguro que deseas desactivar a este departamento?",
        description: "",
        allowClose: true,
    ...
    <div style={{ minHeight: 400, position: "relative" }}>
      <DataGrid
        ...
        slots={{
          toolbar: () => (
            <DataGridToolbar
              tableTitle="Gestión de Departamentos"
              customActionButtons={
