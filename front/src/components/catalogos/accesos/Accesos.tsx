...
export default function Accesos() {
      ...
      getRows: async (params) => {
        ...
        try {
          ...
          const res = await clienteAxios.get(
            "/api/accesos?" + urlParams.toString()
          );
          if (res.data.estado) {
            setError("");
  ...
  const nuevoRegistro = () => {
    navigate("nuevo-acceso");
  };

  const editarRegistro = (ID: string) => {
    navigate(`editar-acceso/${ID}`);
  };

  const verRegistro = (ID: string) => {
    navigate(`detalle-acceso/${ID}`);
  };

  const cambiarEstado = async (ID: string, activo: boolean) => {
    if (!activo) {
      try {
        const res = await clienteAxios.patch(`/api/accesos/${ID}`, {
          activo,
        });
        if (res.data.estado) {
      ...
      confirm({
        title: "¿Seguro que deseas desactivar a este acceso?",
        description: "",
        allowClose: true,
