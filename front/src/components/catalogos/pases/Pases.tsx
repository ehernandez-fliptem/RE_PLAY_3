...
export default function Pases() {
  ...
  const editarRegistro = (ID: string) => {
    navigate(`editar-pase/${ID}`);
  };

  const verRegistro = (ID: string) => {
    navigate(`detalle-pase/${ID}`);
  };

  const cambiarEstado = async (ID: string, activo: boolean) => {
    if (!activo) {
      try {
        const res = await clienteAxios.patch(`/api/pases/${ID}`, {
          activo,
        });
        if (res.data.estado) {
      ...
      confirm({
        title: "¿Seguro que deseas desactivar a este pase?",
        description: "",
        allowClose: true,
