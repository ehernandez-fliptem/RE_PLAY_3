...
export default function NuevoPase() {
  ...
  useEffect(() => {
    const obtenerRegistro = async () => {
      try {
        const res = await clienteAxios.get("/api/pases/form-nuevo");
        if (res.data.estado) {
          const { empresas } = res.data.datos;
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      const res = await clienteAxios.post("api/pases", data);
      if (res.data.estado) {
        enqueueSnackbar("El pase se creó correctamente.", {
          variant: "success",
        });
