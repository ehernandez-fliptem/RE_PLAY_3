...
export default function Visitantes() {
  ...
  const onQrValidate = async (
    ...
    if (!isValid) {
      const message = "QR inv谩lido o no corresponde a un visitante.";
      ...
      if (res.data.estado) {
        ...
        if (puedeAcceder === false) {
          const message = nombre
            ? `Acceso pendiente para ${nombre}. Requiere validaci贸n.`
            : "Acceso pendiente de autorizaci贸n. Requiere validaci贸n.";
  ...
  const verificarSeleccionado = () => {
    ...
    if (!row) {
      enqueueSnackbar("No se encontr贸 el visitante seleccionado.", {
        variant: "warning",
      });
      return;
    }
    ...
    if (row.verificado) {
      enqueueSnackbar("Este visitante ya est谩 verificado.", {
        variant: "info",
      });
      return;
    }
  ...
  const cambiarEstado = async (ID: string, activo: boolean, nombre: string) => {
    if (!activo) {
      confirm({
        title: "縎eguro que deseas restaurar a este visitante?",
        description: nombre,
        allowClose: true,
    ...
    confirm({
      title: "縎eguro que deseas desactivar a este visitante?",
      description: "Al desactivar, se bloquear谩 el acceso y no se podr谩 editar.",
      allowClose: true,
      confirmationText: "Continuar",
    })
      .then(async (result) => {
        ...
        try {
          ...
          if (res.data.estado) {
            ...
            // Si a煤n no est谩 bloqueado, aplicar bloqueo autom谩tico al desactivar.
  ...
  const eliminarPermanente = (ID: string, nombre: string) => {
    confirm({
      title: "縎eguro que deseas eliminar permanentemente este visitante?",
      description: nombre,
      allowClose: true,
      confirmationText: "Continuar",
    })
...
const accionDesbloquear = (ID: string) => {
  ...
  confirm({
    title: "縎eguro que desea desbloquear a este visitante?",
    description: "Esta acci贸n restaura los intentos y habilita el acceso SOLO por hoy.",
    allowClose: true,
    confirmationText: "Continuar",
  })
    .then(async (result) => {
...
const accionBloquear = (ID: string) => {
  ...
  confirm({
    title: "縎eguro que desea bloquear a este visitante?",
    description: "Esta acci贸n bloquea el acceso al sistema para el visitante.",
    allowClose: true,
    confirmationText: "Continuar",
  })
    .then(async (result) => {
    ...
    <div style={{ minHeight: 400, position: "relative" }}>
      <DataGrid
        ...
        slots={{
          toolbar: () => (
            <DataGridToolbar
              tableTitle="Gesti贸n de Visitantes"
              customActionButtons={
                <Fragment>
