...
const resolverPisos = yup.object().shape({
  img_acceso: yup
    .string()
    .test(
      "isValidUri",
      "La imagen de acceso debe ser una URL válida.",
      (value) => {
        if (value) {
...
export default function NuevosAccesos({ setAccesos }: Props) {
  const apiRef = useGridApiRef();
  const formContext = useForm({
    defaultValues: initialValue,
    resolver: yupResolver(resolverPisos),
      ...
      getRows: async (params) => {
        ...
        try {
          ...
          const res = await clienteAxios.get(
            "/api/validacion/accesos?" + urlParams.toString()
          );
          if (res.data.estado) {
            setError("");
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      const res = await clienteAxios.post("api/validacion/acceso", data);
      if (res.data.estado) {
  ...
  const handleDelete = async (ID: string) => {
    try {
      const res = await clienteAxios.delete(`api/validacion/acceso/${ID}`);
      if (res.data.estado) {
        enqueueSnackbar("El acceso se eliminó correctamente.", {
          variant: "success",
        });
        apiRef.current?.dataSource.fetchRows();
      } else {
          ...
          <CardContent>
            ...
            <Box component="div" sx={{ height: 300, width: "100%", mt: 4 }}>
              <DataGrid
                ...
                slots={{
                  toolbar: () => <DataGridToolbar tableTitle="Acceso(s)" />,
                }}
              />
              {error && (
                <ErrorOverlay
