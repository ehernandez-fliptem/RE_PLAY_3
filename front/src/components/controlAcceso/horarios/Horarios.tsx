...
export default function Horarios() {
      ...
      getRows: async (params) => {
        ...
        try {
          ...
          const res = await clienteAxios.get(
            "/api/horarios?" + urlParams.toString()
          );
          if (res.data.estado) {
            setError("");
  ...
  const nuevoRegistro = () => {
    navigate("nuevo-horario");
  };

  const editarRegistro = (ID: string) => {
    navigate(`editar-horario/${ID}`);
  };

  const verRegistro = (ID: string) => {
    navigate(`detalle-horario/${ID}`);
  };

  const cambiarEstado = async (ID: string, activo: boolean) => {
    if (!activo) {
      try {
        const res = await clienteAxios.patch(`/api/horarios/${ID}`, {
          activo,
        });
        if (res.data.estado) {
      ...
      confirm({
        title: "¿Seguro que deseas desactivar este horario?",
        description: "",
        allowClose: true,
    ...
    <div style={{ minHeight: 400, position: "relative" }}>
      <DataGrid
        ...
        slots={{
          toolbar: () => (
            <DataGridToolbar
              tableTitle="Gestión de Horarios"
              customActionButtons={
