...
export default function NuevoHorario() {
  ...
  useEffect(() => {
    const obtenerRegistro = async () => {
      try {
        const res = await clienteAxios.get("/api/horarios/form-nuevo");
        if (res.data.estado) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      ...
      const res = await clienteAxios.post("api/horarios", newData);
      if (res.data.estado) {
        enqueueSnackbar("El horario se creó correctamente.", {
          variant: "success",
        });
