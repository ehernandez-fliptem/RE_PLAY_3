    ...
    const obtenerRegistro = async () => {
      try {
        const res = await clienteAxios.get(
          `/api/asignaciones/form-editar/${ID}`
        );
        if (res.data.estado) {
          const { asignacion, horarios } = res.data.datos;
          formContext.reset({
            ...initialValue,
            ...asignacion,
            periodo: {
              inicio: dayjs(asignacion.periodo.inicio),
              fin: dayjs(asignacion.periodo.fin),
            },
          });
          setUsuario(asignacion.usuario);
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      const newData = {
        ...
        esIndeterminado: data.esIndeterminado,
        periodo: {
          inicio: data.periodo.inicio.startOf("day").toDate(),
          fin: data.periodo.fin.endOf("day").toDate(),
        },
      };
      const res = await clienteAxios.put(`/api/asignaciones/${ID}`, newData);
      if (res.data.estado) {
