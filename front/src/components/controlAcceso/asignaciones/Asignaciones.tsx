...
export default function Asignaciones() {
      ...
      getRows: async (params) => {
        ...
        try {
          ...
          const res = await clienteAxios.get(
            "/api/asignaciones?" + urlParams.toString()
          );
          if (res.data.estado) {
            setError("");
  ...
  const nuevoRegistro = () => {
    navigate("nueva-asignacion");
  };

  const editarRegistro = (ID: string) => {
    navigate(`editar-asignacion/${ID}`);
  };

  const verRegistro = (ID: string) => {
    navigate(`detalle-asignacion/${ID}`);
  };

  const cambiarEstado = async (ID: string, activo: boolean) => {
    if (!activo) {
      try {
        const res = await clienteAxios.patch(`/api/asignaciones/${ID}`, {
          activo,
        });
        if (res.data.estado) {
      ...
      confirm({
        title: "¿Seguro que deseas desactivar esta asignación?",
        description: "",
        allowClose: true,
    ...
    <div style={{ minHeight: 400, position: "relative" }}>
      <DataGrid
        ...
        slots={{
          toolbar: () => (
            <DataGridToolbar
              tableTitle="Gestión de Asignaciones de Horario"
              customActionButtons={
