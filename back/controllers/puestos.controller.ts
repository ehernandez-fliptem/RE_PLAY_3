...
export async function obtenerTodos(req: Request, res: Response): Promise<void> {
    try {
        ...
        const registros = await Puestos.aggregate(aggregation);
...
export async function obtenerTodosActivos(req: Request, res: Response): Promise<void> {
    try {
        ...
        const puestos = await Puestos.find(isMaster ? {} : { _id: id_empresa }, 'nombre puestos activo').sort({ nombre: 1 });
        res.status(200).json({ estado: true, datos: puestos });
    } catch (error: any) {
        log(`${fecha()} ERROR: ${error.name}: ${error.message}\n`);
        res.status(500).send({ estado: false, mensaje: `${error.name}: ${error.message}` });
    }
...
export async function obtenerUno(req: Request, res: Response): Promise<void> {
    try {
        const registro = await Puestos.aggregate([
            {
                $match: {
        ...
        if (!registro[0]) {
            res.status(200).json({ estado: false, mensaje: "Puesto no encontrado" });
            return;
        }
        const empresas = await Empresas.find({ puestos: { $in: [registro[0]._id] } }, 'rfc nombre');
...
export async function modificar(req: Request, res: Response): Promise<void> {
    try {
        const { nombre, identificador } = req.body;
        const id_usuario = (req as UserRequest).userId;
        const registro = await Puestos.findByIdAndUpdate(
            req.params.id,
            {
                $set: {
                    nombre,
...
export async function modificarEstado(req: Request, res: Response): Promise<void> {
    try {
        ...
        const validar_registro = await Usuarios.findOne({ id_puesto: req.params.id });
        if (validar_registro) {
            res.status(200).json({ estado: false, mensaje: 'El puesto está asignado a uno o más usuarios.' });
            return;
        }
        const registro = await Puestos.findByIdAndUpdate(req.params.id, { $set: { activo: !activo } });
        if (!registro) {
            res.status(200).json({ estado: false, mensaje: 'Puesto no encontrado.' });
            return;
        }
