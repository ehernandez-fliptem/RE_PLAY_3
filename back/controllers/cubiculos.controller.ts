...
export async function obtenerTodos(req: Request, res: Response): Promise<void> {
    try {
        ...
        const registros = await Cubiculos.aggregate(aggregation);
...
export async function obtenerTodosActivos(req: Request, res: Response): Promise<void> {
    try {
        ...
        const cubiculos = await Cubiculos.find(isMaster ? {} : { _id: id_empresa }, 'nombre cubiculos activo').sort({ nombre: 1 });
        res.status(200).json({ estado: true, datos: cubiculos });
    } catch (error: any) {
        log(`${fecha()} ERROR: ${error.name}: ${error.message}\n`);
        res.status(500).send({ estado: false, mensaje: `${error.name}: ${error.message}` });
    }
...
export async function obtenerUno(req: Request, res: Response): Promise<void> {
    try {
        const registro = await Cubiculos.aggregate([
            {
                $match: {
        ...
        if (!registro[0]) {
            res.status(200).json({ estado: false, mensaje: "Cubiculo no encontrado" });
            return;
        }
        const empresas = await Empresas.find({ cubiculos: { $in: [registro[0]._id] } }, 'rfc nombre');
...
export async function modificar(req: Request, res: Response): Promise<void> {
    try {
        const { nombre, identificador } = req.body;
        const id_usuario = (req as UserRequest).userId;
        const registro = await Cubiculos.findByIdAndUpdate(
            req.params.id,
            {
                $set: {
                    nombre,
...
export async function modificarEstado(req: Request, res: Response): Promise<void> {
    try {
        ...
        const validar_registro = await Usuarios.findOne({ id_cubiculo: req.params.id });
        if (validar_registro) {
            res.status(200).json({ estado: false, mensaje: 'El cubiculo está asignado a uno o más usuarios.' });
            return;
        }
        const registro = await Cubiculos.findByIdAndUpdate(req.params.id, { $set: { activo: !activo } });
        if (!registro) {
            res.status(200).json({ estado: false, mensaje: 'Cubiculo no encontrado.' });
            return;
        }
