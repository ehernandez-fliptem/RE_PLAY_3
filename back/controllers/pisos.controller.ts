...
export async function obtenerTodosActivos(req: Request, res: Response): Promise<void> {
    try {
        ...
        const pisos = await Pisos.find(isMaster ? {} : { _id: id_empresa }, 'nombre pisos activo').sort({ nombre: 1 });
        res.status(200).json({ estado: true, datos: pisos });
    } catch (error: any) {
        log(`${fecha()} ERROR: ${error.name}: ${error.message}\n`);
        res.status(500).send({ estado: false, mensaje: `${error.name}: ${error.message}` });
    }
...
export async function obtenerUno(req: Request, res: Response): Promise<void> {
    try {
        const registro = await Pisos.aggregate([
            {
                $match: {
        ...
        const empresas = await Empresas.find({ pisos: { $in: [registro[0]._id] } }, 'rfc nombre');
...
export async function modificar(req: Request, res: Response): Promise<void> {
    try {
        const { nombre, identificador } = req.body;
        const id_usuario = (req as UserRequest).userId;
        const registro = await Pisos.findByIdAndUpdate(
            req.params.id,
            {
                $set: {
                    nombre,
        ...
        if (!registro) {
            res.status(200).json({ estado: false, mensaje: 'Piso no encontrado' });
            return;
        }
        res.status(200).json({ estado: true });
    } catch (error: any) {
...
export async function modificarEstado(req: Request, res: Response): Promise<void> {
    try {
        ...
        const validar_registro = await Usuarios.findOne({ id_piso: req.params.id });
        if (validar_registro) {
            res.status(200).json({ estado: false, mensaje: 'El piso está asignado a uno o más usuarios.' });
            return;
        }
        const registro = await Pisos.findByIdAndUpdate(req.params.id, { $set: { activo: !activo } });
        if (!registro) {
            res.status(200).json({ estado: false, mensaje: 'Piso no encontrada.' });
            return;
        }
