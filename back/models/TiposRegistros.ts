...
export interface ITipoRegistro extends Document {
    tipo: number;
    nombre: string;
    color: string;
    descripcion: string;
...
const tiposRegistrosSchema = new Schema<ITipoRegistro>({
...
tiposRegistrosSchema.pre<ITipoRegistro>('save', async function (next) {
    try {
...
const TiposRegistros: Model<ITipoRegistro> = mongoose.model<ITipoRegistro>('tipos_registros', tiposRegistrosSchema);

export default TiposRegistros;