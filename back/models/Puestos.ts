...
puestoSchema.pre<IPuesto>('save', async function (next) {
    try {
...
puestoSchema.plugin(uniqueValidator, {
    type: 'mongoose-unique-validator',
    message: 'El {PATH} `{VALUE}` ya está registrado.',
});

const Puestos: Model<IPuesto> = mongoose.model<IPuesto>('puestos', puestoSchema);
