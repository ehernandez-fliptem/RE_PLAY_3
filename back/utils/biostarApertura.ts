...
async function lockDoor(conexion: any, doorId: string): Promise<{ ok: boolean; message?: string }> {
  const idNum = Number(doorId);
  const ids = Number.isFinite(idNum) ? [idNum] : [doorId];
  const payloads = [
    { DoorCollection: { rows: ids.map((id) => ({ id })) } },
  ...
  const endpoints = ["/api/doors/lock", "/api/doors/close"];
  let lastMsg = "No se pudo cerrar la puerta en BioStar.";
  for (const ep of endpoints) {
    for (const data of payloads) {
      const r = await biostarRequest(conexion as any, { method: "POST", url: ep, data, timeout: 12000 });
      if (r.ok) return { ok: true };
...
async function obtenerDestinoPorAcceso(idAcceso: string) {
  return DispositivosBiostar.findOne({
    activo: true,
...
export async function abrirPuertaPorAccesoBiostar(params: {
  ...
  const destino = await obtenerDestinoPorAcceso(idAcceso);

  if (!destino?.apertura_puerta_id) {
    return { ok: false, skipped: true, message: "Sin puerta destino configurada para este acceso." };
  }

  const acceso = await Accesos.findById(idAcceso, "modo_apertura_biostar segundos_apertura_biostar").lean<{
    modo_apertura_biostar?: "pulso" | "manual";
    segundos_apertura_biostar?: number;
  } | null>();
  const modoApertura = acceso?.modo_apertura_biostar === "manual" ? "manual" : "pulso";
  const segundosApertura = Math.min(30, Math.max(1, Number(acceso?.segundos_apertura_biostar || 3)));
  ...
  if (modoApertura === "pulso") {
    setTimeout(async () => {
      try {
        await lockDoor(conexion, String(destino.apertura_puerta_id).trim());
      } catch {
        // Ignorar errores de cierre para no romper el flujo principal.
      }
    }, segundosApertura * 1000);
  }
...
export async function cerrarPuertaPorAccesoBiostar(params: {
  idAcceso?: Types.ObjectId | string | null;
}): Promise<{ ok: boolean; skipped?: boolean; message?: string }> {
  const idAcceso = params.idAcceso ? String(params.idAcceso) : "";
  if (!idAcceso) return { ok: false, skipped: true, message: "Acceso no definido." };
  ...
  const closeRes = await lockDoor(conexion, String(destino.apertura_puerta_id).trim());
  if (!closeRes.ok) return { ok: false, message: closeRes.message };
